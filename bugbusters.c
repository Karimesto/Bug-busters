...
void clear_transposition_table() {
    for (int i = 0; i < TABLE_SIZE; i++) {
        transpositionTable[i].key[0] = '\0';
        transpositionTable[i].depth = -1000000;
    }
}
/* -------------------------------- */
...
void* thread_worker(void *arg) {
    ...
    if (row == -1) { task->score = -100000000; return NULL; }
...
bool tt_lookup(const char *boardKey, int depth, int *out_value) {
    ...
    if(strcmp(transpositionTable[idx].key,boardKey)==0 &&
...
int minimax(int arr[6][7], int depth, int alpha, int beta,
    ...
    if(lastRow!=-1 && lastCol!=-1 && arr[lastRow][lastCol]!=0){
        if(checkwin(arr,lastRow,lastCol)){
            int winScore = (arr[lastRow][lastCol]==botPlayer ?
                            1000000+depth : -1000000-depth);
...
int botmove_hard(int arr[6][7], int search_depth) {
    ...
    for(int i=0;i<7;i++){
        ...
        tasks[tcount].depth=search_depth;

        if(pthread_create(&threads[tcount],NULL,thread_worker,&tasks[tcount])!=0){
            int row=changefunction(tasks[tcount].arr,col,botPlayer);
            if(row!=-1)
                tasks[tcount].score=minimax(tasks[tcount].arr,tasks[tcount].depth,
                                             -100000000,100000000,0,botPlayer,row,col);
            else
                tasks[tcount].score=-100000000;
        } 
        else tcount++;
...
int main() {
    ...
    while(true){
        ...
        if(col<0 || col>=7){ printf("Invalid column!\n"); continue; }
        int row=changefunction(arr,col,current);
        if(row==-1){ printf("Column full!\n"); continue; }
