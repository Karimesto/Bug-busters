...
int botmove_medium(int arr[6][7]) {
    ...
    for (col = 0; col < 7; col++) {
        if (arr[0][col] == 0) {
            ...
            int maxCount = 0;
            for (int d = 0; d < 4; d++) {
                int drow = direction[d][0], dcol = direction[d][1];
                int count = 1;
                int r = row + drow, c = col + dcol;
                while (r >= 0 && r < 6 && c >= 0 && c < 7 && arr[r][c] == 1) {
                    count++; r += drow; c += dcol;
                }
                r = row - drow; c = col - dcol;
                while (r >= 0 && r < 6 && c >= 0 && c < 7 && arr[r][c] == 1) {
